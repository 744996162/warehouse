...
def get_gt_uid_list_monthly_2015(s1, s2, dbtype="169server"):
    db = get_db(dbtype)
    # result = db.sub_order.find({"status": "支付成功","order_date": {"$gte": 20150101, "$lt": 20150401}}, {"card_no": 1, "phone": 1, "_id":0 })
    uid_list = []
...
def get_phone_set_quarter(s1, s2):
...
def get_phone_set_monthly(s1, s2):
    hb_phone_set = get_hb_consumers_phone_set(s1, s2)
    int_s1 = int(s1)
    int_s2 = int(s2)
    if int_s1 >= 20150301:
        gt_uid_list = get_gt_uid_list_monthly_2015(int_s1, int_s2)
    else:
        gt_uid_list = get_gt_uid_list(int_s1, int_s2)
...
def cal_quarter(s1, s2):
    print(s1, time.ctime())
    file_out = open("result_quarter.dat", "a")
    hb_consumers_num, gt_consumers_num, union_num = get_phone_set_quarter(s1, s2)
    file_out.write(str(s1) + "\t" + str(hb_consumers_num) + "\t" + str(gt_consumers_num) + "\t" + str(union_num) + "\n")
    file_out.close()

def cal_monthly(s1, s2):
    print(s1, time.ctime())
    file_out = open("result_monthly.dat", "a")
    hb_consumers_num, gt_consumers_num, union_num = get_phone_set_monthly(s1, s2)
    file_out.write(str(s1) + "\t" + str(hb_consumers_num) + "\t" + str(gt_consumers_num) + "\t" + str(union_num) + "\n")
    file_out.close()
...
def main_quarter():
    ...
    file_out.close()
    cal_quarter("20130701", "20131001")
    cal_quarter("20131001", "20140101")

    cal_quarter("20140101", "20140401")
    cal_quarter("20140401", "20140701")
    cal_quarter("20140701", "20141001")
    cal_quarter("20141001", "20150101")

    cal_quarter("20150101", "20150401")
...
def main_monthly():
    ...
    file_out.write("time" + "\t" + "hb_consumers" + "\t" + "gt_consuemrs" + "\t" + "union_numbers" + "\n")
    file_out.close()
    # cal_monthly("20150101", "20150201")
    # cal_monthly("20150201", "20150301")
    cal_monthly("20150301", "20150401")
    cal_monthly("20150401", "20150501")
    cal_monthly("20150501", "20150601")
    pass
