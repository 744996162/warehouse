...
def getMondoData(dbtype="local"):
    ...
    result1 = db.sub_order.find({"status": "支付成功", "order_date":{"$gte":20150226, "$lt":20150501}}, {"uid": 1, "card_no": 1, "_id":0 })
    d = defaultdict(list)
    count = 1
    ...
    # del result1
    #
    # result2 = db.sub_order_his.find({"status": "支付成功", "order_date":{"$gte":20130601, "$lt":20141001}}, {"uid": 1, "card_no": 1, "_id":0 })
    #
    # for i in result2:
    #     count += 1
    #     uid = str(i["uid"])
    ...
    # del result2
    #
    # result3 = db.sub_order_his.find({"status": "支付成功", "order_date":{"$gte":20141001, "$lt":20150226}}, {"uid": 1, "card_no": 1, "_id":0 })
    # for i in result3:
    #     count += 1
    #     uid = str(i["uid"])
    #     card_no = str(i["card_no"])
