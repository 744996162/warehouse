...
def store_all(json_str,store_path):
    ...
    ###change
    flightData_all_list = datas["qunar"]
...
def get_one_store_test():
    url_list = getUrl("2015-02-27",detal_days=10)
    # print(url_list[0])
    org = url_list[0][0]
    dst = url_list[0][1]
...
def getToday(format='%Y%m%d'):
    ...
    return yes_time.strftime(format)

def get_all_store_test(date_str, detal_days=30):

    today = getToday()

    # file_path = today
    file_path="/homezhangc/data/flight_price_analysis_qunar/"+today
    if os.path.exists(today):
        pass
    else:
        os.mkdir(today)

    url_detail_list = getUrl(date_str, detal_days=30)
...
if __name__=="__main__":
    ...
    date_str_today=getToday(format='%Y-%m-%d')
    get_all_store_test(date_str_today)
    # get_one_store_test()