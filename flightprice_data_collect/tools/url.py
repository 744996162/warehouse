...
class Url(City):
    def __init__(self, source="qunar"):
        City.__init__(self)
        # self.url_header="http://search.rsscc.cn/flight/domestic?src=ctrip"
        self.url_header = "http://search.rsscc.cn/flight/domestic?src="+source

    def getUrl(self, org, dst, date_str):
        header = self.url_header
        url_out = header + "&org="+org+"&dst=" + dst + "&date=" + date_str
        return url_out
    ...
    def getAllUrl(self, start_day_str, diff_day, date_format="%Y-%m-%d"):
        url_list = []
        date_list = self.getDateList(start_day_str, diff_day, date_format=date_format)
...
if __name__ == "__main__":
    t = Url("ctrip")
    url_list = t.getAllUrl("2015-02-27", 10)
    # print(len(url_list))
    for i in url_list:
        print(i)
    print(len(url_list))
