        ...
        if Mysql._mysql_pool is None:
         #   MySQLdb.install_as_MySQLdb()
            if dbtype == '79bi':
                ...
                Mysql._mysql_pool = PooledDB(creator=MySQLdb, mincached=1, maxcached=20, host=_conf.get_mysql('host91bi'), port=string.atoi(_conf.get_mysql('port91bi')), user=_conf.get_mysql('user91bi'), passwd=_conf.get_mysql('password91bi'), database=_conf.get_mysql('database91bi'), charset='utf8', cursorclass=DictCursor)
            else:
