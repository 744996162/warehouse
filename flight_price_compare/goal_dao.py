...
class CheapPriceUpdateDao(Mysql):
    def __init__(self, dbtype="91bi"):
...
class ExpensivePriceUpdateDao(Mysql):
    def __init__(self, dbtype="91bi"):
        Mysql.__init__(self, dbtype=dbtype)
    ...
    def insert_data(self,_data):
        if (_data==None) or (len(_data)<=0):
            ...
            db_name='flight_price_percent'
            sql='insert into '+db_name+' (flightnum,airlineno,hbgj_percent,xc_percent,qunar_percent,datatime,updatetime)'\
                                         'values (%s, %s,%s,%s, %s, %s,%s)'
            args = []
            i = 0
            for data in _data:
                i += 1
                arg = [data.flightnum,data.airlineno,data.hbgj_percent,data.xc_percent,data.qunar_percent,data.datatime,data.updatetime]
                args.append(arg)
                if i%100 == 0 or i == len(_data):
                    self.insert_many(sql, args)
                    args = []
